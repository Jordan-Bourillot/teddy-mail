...
export function computeInsights(mails, now = new Date()) {
    ...
    for (const m of mails) {
        ...
        if (t > oneWeekAgo)
            trackersThisWeek += m.trackersBlocked;
        if (!m.read)
            unread += 1;
        if (m.snoozedUntil && new Date(m.snoozedUntil) > now)
            snoozed += 1;
        if (m.starred)
            starred += 1;
        ...
        if (cur) {
            ...
            const entry = { email: m.from.email, count: 1 };
            if (m.from.name !== undefined)
                entry.name = m.from.name;
