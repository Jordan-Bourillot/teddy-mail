 ...
 */
export function neutralizeTrackers(html: string): { sanitized: string; blocked: number } {
  let blocked = 0;
  // Strip tiny images (typical tracker pixel signatures).
  const sanitized = html.replace(
    /<img\b[^>]*?(?:width\s*=\s*["']?1["']?|height\s*=\s*["']?1["']?|style\s*=\s*["'][^"']*?(?:width\s*:\s*1px|height\s*:\s*1px))[^>]*>/gi,
    () => {
      blocked += 1;
